...
static struct platform_device mdm_monarudo_device = {
	.name		= "mdm2_modem",
	.id		= -1,
...
static void __init monarudo_cdp_init(void)
{
	...
	mdm_monarudo_device.dev.platform_data = &mdm_platform_data;
	if (system_rev >= XC) {
		mdm_monarudo_device.resource[5].start = MDM2AP_HSIC_READY_XC;
		mdm_monarudo_device.resource[5].end = MDM2AP_HSIC_READY_XC;
		mdm_monarudo_device.resource[6].start = AP2MDM_WAKEUP_XC;
		mdm_monarudo_device.resource[6].end = AP2MDM_WAKEUP_XC;
	}
	platform_device_register(&mdm_monarudo_device);
