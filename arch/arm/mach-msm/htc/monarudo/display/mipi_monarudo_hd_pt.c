...
static int __init mipi_monarudo_hd_pt_init(void)
{
	if (panel_type == PANEL_ID_DLX_SHARP_RENESAS) {
		mipi_video_sharp_init();
		pr_info("%s panel ID = PANEL_ID_DLX_SHARP_RENESAS\n", __func__);
	} else if (panel_type == PANEL_ID_DLX_SONY_RENESAS) {
		mipi_video_sony_init();
		pr_info("%s panel ID = PANEL_ID_DLX_SONY_RENESAS\n", __func__);
	} else {
...
late_initcall(mipi_monarudo_hd_pt_init);
